package com.goldax.goldax.ui.pick.recycler;
...
public class PickAdapter extends MainBaseAdapter {
    private static final String TAG = PickAdapter.class.getSimpleName();
    ...
    public PickAdapter(ArrayList<ListResult.ResultData> itemList, View.OnClickListener itemClickListener) {
    ...
    public void onBindViewHolder(@NonNull MainBaseViewHolder holder, int position) {
        if (holder instanceof DummyViewHolder) {
            holder.bind(position, DataConst.FRAGMENT_TYPE.PICK);
        } else {
